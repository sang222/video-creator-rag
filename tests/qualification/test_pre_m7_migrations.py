...
def test_m0_to_m6_schema_head_tables_and_json_defaults(
    engine, qualification_factory
) -> None:
    ...
    assert flow.version.context_refs == [
        {"type": "context_pack_snapshot", "id": "ctx-1"}
    ]
    assert flow.version.claim_refs == [{"type": "claim", "id": "cl-1"}]


def test_fk_unique_constraints_and_failed_transaction_rollback(
    ...
    try:
        ProviderRegistryService(db_session).create_entry(
            data=ProviderRegistryEntryCreate(
                provider_key="openai", provider_name="Duplicate", provider_type="LLM"
            )
        )
    except Exception:
        db_session.rollback()
