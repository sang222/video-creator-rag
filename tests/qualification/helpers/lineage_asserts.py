...
from .qualification_asserts import assert_file_ref_is_verified
...
def assert_m6_render_lineage(session, run) -> None:
    ...
    source_manifest = session.get(
        SourceManifestSnapshot, run.source_manifest_snapshot_id
    )
    ...
    assert (
        source_manifest.source_manifest_blob["provider_classification_summary"][
            "real_provider_calls"
        ]
        == 0
    )
