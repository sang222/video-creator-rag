...
from app.services.m10_2 import LOCAL_RENDERER_CAPABILITY, LONG_FORM_FINAL_RENDER, MediaRenderJobRouterService
...
def test_native_renderer_owns_local_render_capabilities():
    ...
    native = [item for item in entries if item["provider_key"] == "native_ffmpeg_renderer"]
    assert native
...
def test_render_routing_catalog_targets_native_renderer():
    ...
    render_jobs = {"LONG_FORM_FINAL_RENDER", "SHORT_RENDER", "THUMBNAIL_RENDER", "DIAGRAM_CARD_RENDER"}
    routed = {item["job_type"]: item["provider_key"] for item in entries if item["job_type"] in render_jobs}
    assert all(routed.get(job) == "native_ffmpeg_renderer" for job in render_jobs - {"LONG_FORM_FINAL_RENDER"})


def test_long_form_router_declares_native_local_authority():
    source = Path(MediaRenderJobRouterService.__module__.replace(".", "/") + ".py").read_text(encoding="utf-8")
