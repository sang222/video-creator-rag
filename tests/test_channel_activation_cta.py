...
def _make_company(session: Session) -> Company:
    ...
    company = svc.create_company(
        name=f"Activation Test Co {uuid.uuid4().hex[:6]}",
        slug=f"activation-test-{uuid.uuid4().hex[:6]}",
    )
    session.flush()
    return company
...
def _make_channel(
    session: Session, company_id: uuid.UUID, *, status: str = "draft"
) -> ChannelWorkspace:
...
def _profile_input(session: Session, contract: dict):
    ...
    return profile_input.model_copy(
        update={
            "display_name": contract["channel_identity"].get("channel_name")
            or "Test Channel",
    ...
    profile = ChannelProfileService(session).create_profile_version(
        ...
        data=ChannelProfileVersionCreate(
            profile_input=_profile_input(
                session, contract or _complete_contract_payload()
            )
        ),
    )
    compiled = compiler.compile(
        profile_version_id=profile.id, correlation_id=correlation_id
    )
...
class TestActivationSucceedsWhenContractComplete:
    ...
    def test_activation_sets_channel_active(self, db_session: Session) -> None:
        ...
        result = ChannelProfileService(db_session).activate_snapshot(
...
class TestActivationBlockedWhenContractPartial:
    ...
    def test_activation_raises_on_partial_contract(self, db_session: Session) -> None:
        company = _make_company(db_session)
        channel = _make_channel(db_session, company.id, status="draft")
        _, snapshot = _compile_snapshot(
            db_session, channel, contract=_partial_contract_payload()
        )
...
class TestActivationBlockedWhenNoSnapshot:
    ...
    def test_activation_endpoint_blocks_channel_without_snapshot(
        ...
        response = TestClient(create_app()).post(
            f"/channels/{channel.id}/activate", json={}
        )
...
class TestActivationDoesNotMutateVideoProjectSnapshots:
    ...
    def test_activation_preserves_project_snapshots(self, db_session: Session) -> None:
        ...
        ChannelProfileService(db_session).activate_snapshot(
