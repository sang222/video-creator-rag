...
class PostPublishObservationWindow(Base):
    ...
    expected_check_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "uploaded_video_id",
            "observation_window",
            name="uq_post_publish_windows_video_window",
        ),
...
class DiagnosticTaxonomyVersion(Base):
    ...
    taxonomy_blob: Mapped[dict[str, Any]] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint(
            "taxonomy_key",
            "version",
            name="uq_diagnostic_taxonomy_versions_key_version",
        ),
...
class PostPublishHealthRun(Base):
    ...
    retention_curve_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("retention_curve_snapshots.id")
    )
    traffic_source_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("traffic_source_snapshots.id")
    )
    engagement_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("engagement_snapshots.id")
    )
    ...
    def strategic_lineage(self) -> dict[str, Any] | None:
        value = (self.technical_appendix or {}).get("strategic_lineage")
    ...
    __table_args__ = (
        ...
        Index(
            "ix_post_publish_health_runs_channel_workspace_id", "channel_workspace_id"
        ),
...
class PackagingDiagnosticRun(Base):
    ...
    __table_args__ = (
        Index(
            "ix_packaging_diagnostic_runs_health_run_id", "post_publish_health_run_id"
        ),
...
class RetentionDiagnosticRun(Base):
    ...
    scene_alignment: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    confidence_level: Mapped[str] = mapped_column(
        String(40), nullable=False, default="UNKNOWN"
    ...
    __table_args__ = (
        Index(
            "ix_retention_diagnostic_runs_health_run_id", "post_publish_health_run_id"
        ),
...
class EngagementDiagnosticRun(Base):
    ...
    __table_args__ = (
        Index(
            "ix_engagement_diagnostic_runs_health_run_id", "post_publish_health_run_id"
        ),
...
class PolicyRightsDiagnosticRun(Base):
    ...
    __table_args__ = (
        Index(
            ...
            "post_publish_health_run_id",
        ),
        Index(
            "ix_policy_rights_diagnostic_runs_uploaded_video_id", "uploaded_video_id"
        ),
...
class FailureTraceReport(Base):
    ...
    secondary_suspected_causes: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    confidence_level: Mapped[str] = mapped_column(
        String(40), nullable=False, default="UNKNOWN"
    ...
    severity: Mapped[str] = mapped_column(String(40), nullable=False, default="INFO")
    evidence_plain_text: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    operator_summary: Mapped[str] = mapped_column(Text, nullable=False)
    operator_report: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, default=dict
    )
    next_action: Mapped[str | None] = mapped_column(Text)
    do_not_do: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)
...
class RecoveryProposal(Base):
    ...
    id: Mapped[uuid.UUID] = uuid_pk()
    failure_trace_report_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("failure_trace_reports.id"), nullable=False
    )
    uploaded_video_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("uploaded_videos.id"), nullable=False
    )
    ...
    proposal_state: Mapped[str] = mapped_column(
