...
class MediaProviderRoleProfile(Base):
    ...
    is_real_provider: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    supports_real_execution: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    monthly_budget_assumption: Mapped[dict[str, Any]] = mapped_column(
...
class ProviderCapabilityMatrixEntry(Base):
    ...
    provider_key: Mapped[str] = mapped_column(
        String(160),
        ForeignKey("media_provider_role_profiles.provider_key"),
        nullable=False,
    )
    provider_type: Mapped[str] = mapped_column(String(80), nullable=False)
    ...
    supported_aspect_ratios: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    supported_outputs: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, default=list
    )
...
class MediaRenderRoutingDecision(Base):
    ...
    budget_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("media_provider_budget_snapshots.id")
    )
...
class MediaProviderBudgetPolicy(Base):
    ...
    current_mode: Mapped[str] = mapped_column(
        String(40), nullable=False, default="QUALITY_FIRST_250"
    )
...
class LongFormRenderPackage(Base):
    ...
    ai_hero_asset_refs: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    approved_asset_refs: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    thumbnail_variant_refs: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    music_sfx_refs: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    ...
    final_renderer_required: Mapped[bool] = mapped_column(
...
class AIHeroAsset(Base):
    ...
    provider_type: Mapped[str] = mapped_column(
        String(80), nullable=False, default="AI_VIDEO_HERO_PROVIDER"
    )
    provider_key: Mapped[str | None] = mapped_column(String(160))
...
class ThumbnailVariant(Base):
    ...
    provider_type: Mapped[str] = mapped_column(
        String(80), nullable=False, default="LOCAL_RENDERER_CAPABILITY"
    )
    provider_key: Mapped[str | None] = mapped_column(String(160))
...
class FinalMediaRef(Base):
    ...
    lineage_artifact_version_id: Mapped[uuid.UUID | None] = mapped_column(
