...
class LongFormAnalyticsWindow(Base):
    ...
    policy_snapshot_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("compiled_channel_policy_snapshots.id"), nullable=False)
    channel_profile_version_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("channel_profile_versions.id"), nullable=False)
    destination_binding_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    destination_binding_fingerprint: Mapped[str] = mapped_column(String(64), nullable=False)
    target_market_lineage: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, default=dict)
    production_lane: Mapped[str] = mapped_column(String(40), nullable=False, default="LONG_FORM")
    content_mode: Mapped[str] = mapped_column(String(40), nullable=False)
    series_plan_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("series_plans.id"))
    series_run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("series_runs.id"))
    episode_number: Mapped[int | None] = mapped_column(Integer)
    standalone_reason_code: Mapped[str | None] = mapped_column(String(160))
    metric_authority: Mapped[str] = mapped_column(String(40), nullable=False, default="YOUTUBE_OWNER")
    ...
    minimum_maturity_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    ...
    post_publish_health_run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("post_publish_health_runs.id"))
    ...
    __table_args__ = (
        UniqueConstraint("uploaded_video_id", "metric_authority", "window_type", name="uq_long_form_analytics_window_authority"),
        UniqueConstraint("canonical_input_hash", name="uq_long_form_analytics_window_input_hash"),
        Index("ix_long_form_analytics_windows_due", "state", "scheduled_for", "next_attempt_at"),
