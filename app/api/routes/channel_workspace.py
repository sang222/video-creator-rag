...
def create_router() -> APIRouter:
    ...
    def create_minimal_market_channel(data: MinimalMarketChannelInit) -> dict[str, Any]:
        try:
            with session_scope() as session:
                channel = MarketChannelGovernanceService(
                    session
                ).create_minimal_channel(data)
                return {
    ...
    def run_target_market_draft(channel_id: uuid.UUID) -> TargetMarketProfileDraft:
        try:
            with session_scope() as session:
                return MarketChannelGovernanceService(
                    session
                ).run_market_research_draft(channel_id)
        except Exception as exc:
            raise _as_http_error(exc) from exc
    ...
    def read_target_market_draft(channel_id: uuid.UUID) -> TargetMarketProfileDraft:
        try:
            with session_scope() as session:
                return MarketChannelGovernanceService(session).get_market_draft(
                    channel_id
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc
    ...
    def approve_target_market_draft(
        ...
        try:
            ...
            authoritative = data.model_copy(update={"reviewer": str(actor.actor_id)})
            with session_scope() as session:
    ...
    def create_channel_init_draft(data: ChannelInitDraftCreate) -> ChannelInitDraftRead:
        try:
            with session_scope() as session:
                ...
                return ChannelInitDraftRead.model_validate(
                    _channel_init_draft(draft, None)
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc
    ...
    def get_channel_init_draft(draft_id: uuid.UUID) -> ChannelInitDraftRead:
        try:
            with session_scope() as session:
                ...
                return ChannelInitDraftRead.model_validate(
                    _channel_init_draft(draft, latest)
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc

    @router.post(
        "/channel-init-drafts/{draft_id}/research",
        response_model=ChannelContractDraftRead,
    )
    def research_channel_init_draft(
        ...
        try:
            with session_scope() as session:
                ...
                return ChannelContractDraftRead.model_validate(
                    _channel_contract_draft(contract_draft)
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc

    @router.post(
        "/channel-init-drafts/{draft_id}/review",
        response_model=ChannelContractDraftRead,
    )
    ...
    @router.post(
        "/channel-init-drafts/{draft_id}/compile",
        response_model=ChannelInitCompileResult,
    )
    def compile_channel_init_draft(
        ...
        try:
            with session_scope() as session:
                ...
                result = ChannelContractCompiler(session).compile(
                    draft_id, correlation_id=request.correlation_id
                )
    ...
    @router.get(
        "/channel-init-drafts/{draft_id}/contract-preview",
        response_model=ChannelContractPreviewRead,
    )
    def preview_channel_init_contract(
        draft_id: uuid.UUID,
    ) -> ChannelContractPreviewRead:
        try:
            with session_scope() as session:
    ...
    @router.post(
        "/companies/{company_id}/channels", response_model=ChannelWorkspaceRead
    )
    def create_channel(
        company_id: uuid.UUID, data: ChannelWorkspaceCreate
    ) -> ChannelWorkspaceRead:
        try:
            with session_scope() as session:
    ...
    @router.get(
        "/companies/{company_id}/channels", response_model=list[ChannelWorkspaceRead]
    )
    def list_channels(company_id: uuid.UUID) -> list[ChannelWorkspaceRead]:
        with session_scope() as session:
            ...
            return [
                ChannelWorkspaceRead.model_validate(_channel(channel))
                for channel in channels
            ]

    @router.get("/channels")
    def list_dashboard_channels(
    ...
    @router.get(
        "/channels/{channel_id}/workspace", response_model=ChannelWorkspaceDashboardRead
    )
    def get_channel_workspace_dashboard(
        channel_id: uuid.UUID,
    ) -> ChannelWorkspaceDashboardRead:
        try:
            with session_scope() as session:
    ...
    @router.get(
        "/channels/{channel_id}/upload-tasks", response_model=HumanUploadTaskListRead
    )
    ...
    @router.get(
        "/channels/{channel_id}/uploaded-videos", response_model=UploadedVideoListRead
    )
    ...
    @router.get(
        "/channels/{channel_id}/publish-ledger", response_model=PublishLedgerRead
    )
    ...
    @router.post(
        "/channels/{channel_id}/lifecycle-decision",
        response_model=ChannelLifecycleDecisionRead,
    )
    def create_channel_lifecycle_decision(
        ...
        try:
            ...
            with session_scope() as session:
                decision = M11ChannelLifecycleService(session).decide(
    ...
    @router.get(
        "/channels/{channel_id}/localization-config",
        response_model=ChannelLocalizationConfig,
    )
    def get_channel_localization_config(
        channel_id: uuid.UUID,
    ) -> ChannelLocalizationConfig:
        try:
            with session_scope() as session:
    ...
    @router.get(
        ...
        response_model=ChannelPublishTimingPolicyRead,
    )
    def get_channel_publish_timing_policy(
        channel_id: uuid.UUID,
    ) -> ChannelPublishTimingPolicyRead:
        try:
            with session_scope() as session:
    ...
    @router.post(
        "/channels/{channel_id}/memberships", response_model=ChannelMembershipRead
    )
    def assign_membership(
        channel_id: uuid.UUID, data: ChannelMembershipCreate
    ) -> ChannelMembershipRead:
        try:
            with session_scope() as session:
    ...
    @router.post(
        "/channels/{channel_id}/profile-versions",
        response_model=ChannelProfileVersionRead,
    )
    ...
    @router.get(
        "/channels/{channel_id}/profile-versions",
        response_model=list[ChannelProfileVersionRead],
    )
    def list_profile_versions(channel_id: uuid.UUID) -> list[ChannelProfileVersionRead]:
        with session_scope() as session:
            ...
            return [
                ChannelProfileVersionRead.model_validate(_profile(profile))
                for profile in profiles
            ]

    @router.get(
        "/channels/{channel_id}/profile-versions/active",
        response_model=ChannelProfileVersionRead | None,
    )
    def get_active_profile_version(
        channel_id: uuid.UUID,
    ) -> ChannelProfileVersionRead | None:
        with session_scope() as session:
            profile = ChannelProfileService(session).get_active_profile_version(
                channel_id
            )
            return (
                ChannelProfileVersionRead.model_validate(_profile(profile))
    ...
    @router.post(
        "/channels/{channel_id}/profile-versions/draft-from-active",
        response_model=ChannelProfileVersionRead,
    )
    def create_profile_draft_from_active(
        channel_id: uuid.UUID,
    ) -> ChannelProfileVersionRead:
        try:
            with session_scope() as session:
                profile = ChannelProfileService(session).create_draft_from_active(
                    channel_id=channel_id
                )
                return ChannelProfileVersionRead.model_validate(_profile(profile))
        except Exception as exc:
    ...
    def preview_profile_compile(profile_version_id: uuid.UUID) -> dict[str, Any]:
        try:
            with session_scope() as session:
                return ChannelProfileService(session).preview_compile(
                    profile_version_id
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc
    ...
    def diff_profile_versions(
        ...
        try:
            with session_scope() as session:
                return ChannelProfileService(session).semantic_diff(
                    profile_version_id, other_profile_version_id
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc

    @router.post(
        "/profile-versions/{profile_version_id}/compile",
        response_model=ChannelProfileCompileResult,
    )
        ...
        try:
            with session_scope() as session:
                ...
                return ChannelProfileCompiler(session).compile(
                    ...
                    or f"api-compile-{profile_version_id}",
                )
        except Exception as exc:
            raise _as_http_error(exc) from exc
    ...
    def compile_channel_policy_snapshot(
        ...
        try:
            with session_scope() as session:
                profiles = ChannelProfileService(session).list_profile_versions(
                    channel_id
                )
                if not profiles:
                    raise NotFoundError(
                        f"profile version not found for channel: {channel_id}"
                    )
                request = data or ChannelProfileCompileRequest()
                compiled = ChannelProfileCompiler(session).compile(
                    profile_version_id=profiles[0].id,
                    correlation_id=request.correlation_id
                    or f"api-channel-compile-{channel_id}",
                )
                snapshot = PolicySnapshotService(session).get_snapshot(
    ...
    def submit_profile_for_approval(
        ...
        try:
            with session_scope() as session:
                profile = ChannelProfileService(session).submit_for_approval(
                    profile_version_id
                )
                return ChannelProfileVersionRead.model_validate(_profile(profile))
        except Exception as exc:
    ...
    @router.post(
        "/policy-snapshots/{snapshot_id}/activate", response_model=SnapshotRead
    )
    ...
    def get_channel_policy_snapshot(channel_id: uuid.UUID) -> dict[str, Any] | None:
        try:
            with session_scope() as session:
                ...
                snapshot = PolicySnapshotService(
                    session
                ).get_active_snapshot_for_channel(channel_id) or (
                    snapshots[0] if snapshots else None
                )
                return (
                    _snapshot_with_contract_state(snapshot)
    ...
    def activate_channel(
        channel_id: uuid.UUID, data: ChannelActivateRequest | None = None
    ) -> dict[str, Any]:
        try:
            with session_scope() as session:
                ...
                if request.snapshot_id is not None:
                    ...
                    if (
                        ...
                        and snapshot.channel_workspace_id != channel_id
                    ):
                        raise ValidationFailureError(
                            "policy snapshot does not belong to selected channel"
                        )
                else:
                    snapshots = PolicySnapshotService(session).list_snapshots(
                        channel_id
                    )
                    snapshot = snapshots[0] if snapshots else None
                if snapshot is None:
                    raise NotFoundError(
                        f"policy snapshot not found for channel: {channel_id}"
                    )
                activated = ChannelProfileService(session).activate_snapshot(
    ...
    @router.get(
        "/channels/{channel_id}/active-policy-snapshot",
        response_model=SnapshotRead | None,
    )
    def get_active_policy_snapshot(channel_id: uuid.UUID) -> SnapshotRead | None:
        with session_scope() as session:
            ...
            return (
                SnapshotRead.model_validate(_snapshot(snapshot))
                if snapshot is not None
                else None
            )
