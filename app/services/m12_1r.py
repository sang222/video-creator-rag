...
class MockRuntimePurgeService:
    ...
    def _counts(self) -> dict[str, int]:
        return {
            ...
            "mock_provider_attempts": self._count(
                ...
                .where(_mock_provider_key(ProviderAttempt.provider_key))
            ),
            ...
            "mock_production_runs": self._count(
                select(func.count())
                .select_from(ProductionArtifactRun)
                .where(ProductionArtifactRun.run_mode.in_(["MOCK", "LOCAL_FIXTURE"]))
            ),
            "local_ffmpeg_render_jobs": self._count(
                select(func.count())
                .select_from(MediaRenderJob)
                .where(MediaRenderJob.renderer_key == "LOCAL_FFMPEG")
            ),
            "mock_analytics_runs": self._count(
                ...
                .where(
                    or_(
                        AnalyticsSyncRun.sync_mode == "MOCK",
                        _mock_provider_key(AnalyticsSyncRun.provider_key),
                    )
                )
            ),
        }
    ...
    def _disable_provider_rows(self) -> None:
        for row in self.session.scalars(
            select(ProviderRegistryEntry).where(
                _mock_provider_key(ProviderRegistryEntry.provider_key)
            )
        ):
            row.status = "DISABLED"
    ...
    def _quarantine_provider_refs(self) -> None:
        for row in self.session.scalars(
            select(CredentialReference).where(
                _mock_provider_key(CredentialReference.provider_key)
            )
        ):
            ...
            row.metadata_ = _with_purge_marker(row.metadata_)
        for row in self.session.scalars(
            select(QuotaAccount).where(_mock_provider_key(QuotaAccount.provider_key))
        ):
            row.provider_key = PURGED_PROVIDER_KEY
            row.status = "DISABLED"
            row.metadata_ = _with_purge_marker(row.metadata_)
        for row in self.session.scalars(
            select(RetryPolicy).where(_mock_provider_key(RetryPolicy.provider_key))
        ):
            row.provider_key = PURGED_PROVIDER_KEY
            row.status = "DISABLED"
    ...
    def _quarantine_append_only_mock_rows(self) -> None:
        with self._disabled_trigger("provider_attempts"):
            for row in self.session.scalars(
                select(ProviderAttempt).where(
                    _mock_provider_key(ProviderAttempt.provider_key)
                )
            ):
                row.provider_key = PURGED_PROVIDER_KEY
                ...
                row.error_message_redacted = (
                    "Mock provider attempt purged from production-active runtime."
                )
                row.metadata_ = _with_purge_marker(row.metadata_)
            self.session.flush()
        with self._disabled_trigger("cost_events"):
            for row in self.session.scalars(
                select(CostEvent).where(_mock_provider_key(CostEvent.provider_key))
            ):
                row.provider_key = PURGED_PROVIDER_KEY
        ...
        with self._disabled_trigger("quota_events"):
            for row in self.session.scalars(
                select(QuotaEvent).where(_mock_provider_key(QuotaEvent.provider_key))
            ):
                row.provider_key = PURGED_PROVIDER_KEY
        ...
        with self._disabled_trigger("provider_health_snapshots"):
            for row in self.session.scalars(
                select(ProviderHealthSnapshot).where(
                    _mock_provider_key(ProviderHealthSnapshot.provider_key)
                )
            ):
                row.provider_key = PURGED_PROVIDER_KEY
        ...
        with self._disabled_trigger("credential_health_snapshots"):
            for row in self.session.scalars(
                select(CredentialHealthSnapshot).where(
                    _mock_provider_key(CredentialHealthSnapshot.provider_key)
                )
            ):
                row.provider_key = PURGED_PROVIDER_KEY
                row.reason_codes = _append_reason(row.reason_codes)
    ...
    def _block_production_runs(self) -> None:
        for row in self.session.scalars(
            select(ProductionArtifactRun).where(
                ProductionArtifactRun.run_mode.in_(["MOCK", "LOCAL_FIXTURE"])
            )
        ):
            row.run_mode = "REAL_DISABLED"
            row.status = "BLOCKED"
    ...
    def _block_render_jobs(self) -> None:
        for row in self.session.scalars(
            select(MediaRenderJob).where(MediaRenderJob.renderer_key == "LOCAL_FFMPEG")
        ):
            row.renderer_key = PURGED_RENDERER_KEY
            row.status = "BLOCKED"
            row.error_code = PURGE_REASON
            row.error_message_redacted = (
                "Local fixture renderer job purged from production-active runtime."
            )
            row.reason_codes = _append_reason(row.reason_codes)
            row.metadata_ = _with_purge_marker(row.metadata_)
...
class _DisabledTrigger:
    ...
    def __enter__(self) -> None:
        self.session.execute(
            text(f"ALTER TABLE {self.table_name} DISABLE TRIGGER {self.trigger_name}")
        )
    ...
    def __exit__(self, exc_type, exc, traceback) -> None:
        if exc_type is None:
            self.session.execute(
                text(
                    f"ALTER TABLE {self.table_name} ENABLE TRIGGER {self.trigger_name}"
                )
            )
