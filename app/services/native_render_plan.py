...
def canonical_caption_cues(timeline: CanonicalMediaTimeline | None) -> list[Any]:
    ...
    return [
        ...
        for cue in list(getattr(segment, "caption_cues", []) or [])
    ]


def canonical_caption_compilation_hash(
    timeline: CanonicalMediaTimeline, cues: list[Any]
) -> str:
...
class NativeRenderPlanValidator:
    ...
    def validate(
        ...
        *,
        ...
        required = [
            plan.channel_profile_version_id,
            plan.effective_context_snapshot_id,
            plan.effective_context_hash,
            plan.script_hash,
            plan.srt_hash,
            plan.visual_plan_hash,
        ]
        results.append(
            self._gate(
                "NativeRenderPlanCompletenessGate",
                all(required) and bool(plan.scenes),
                "PLAN_INCOMPLETE",
            )
        )
        results.append(
            self._gate(
                "FrozenContextReferenceGate",
                bool(
                    plan.channel_profile_version_id
                    and plan.effective_context_snapshot_id
                    and plan.effective_context_hash
                ),
                "FROZEN_CONTEXT_REF_MISSING",
            )
        )
        results.append(
            self._gate(
                "FormatIdentityReferenceGate",
                plan.format_identity_status == "APPROVED"
                and bool(plan.format_identity_contract_hash),
                "FORMAT_IDENTITY_NOT_APPROVED",
            )
        )
        originality_ok = (
            not plan.production_eligible
        ) or plan.final_originality_gate == "PASS"
        results.append(
            self._gate(
                "FinalOriginalityReferenceGate",
                originality_ok and bool(plan.episode_originality_manifest_hash),
                "FINAL_ORIGINALITY_NOT_PASS",
            )
        )
        results.append(
            self._gate(
                "NoCharacterRenderGate",
                plan.character_policy_mode == "NO_CHARACTER",
                "CHARACTER_POLICY_CONFLICT",
            )
        )
        ordered = sorted(plan.scenes, key=lambda s: s.narration_start_ms)
        overlap = any(
            a.narration_end_ms > b.narration_start_ms
            for a, b in zip(ordered, ordered[1:])
        )
        results.append(
            self._gate("SceneTimelineGate", not overlap, "SCENE_TIMELINE_OVERLAP")
        )
        if plan.production_package_schema_version == "v2":
            package_authority_ok = bool(
                ...
                and plan.package_id == plan.production_package_artifact_version_id
            )
            results.append(
                self._gate(
        ...
        legacy_caption_exists = (
            Path(plan.srt_ref).is_file() if execution else bool(plan.srt_ref)
        )
        results.append(
            self._gate(
                "CaptionTimelineGate",
        ...
        if plan.temporal_authority_mode == "CANONICAL_STRICT":
            ...
            results.append(
                self._gate(
                    ...
                    refs_present,
                    "TEMPORAL_CANONICAL_TIMELINE_REQUIRED",
                )
            )
            results.append(
                self._gate(
                    "ParallelTimingInputGate",
                    not plan.parallel_timing_inputs,
                    "TEMPORAL_PARALLEL_TIMELINE_DETECTED",
                    plan.parallel_timing_inputs,
                )
            )
            results.append(
                self._gate(
            ...
            if canonical_timeline is not None:
                ...
                actual_hash = stable_hash(
                    canonical_timeline.model_dump(
                        mode="json", exclude={"timeline_hash"}
                    )
                )
                hash_ok = (
                ...
                results.append(
                    self._gate(
                        ...
                        hash_ok,
                        "TEMPORAL_TIMELINE_HASH_MISMATCH",
                    )
                )
                results.append(
                    self._gate(
                        "CanonicalAudioAssetGate",
                        plan.canonical_audio_asset_ref
                        == canonical_timeline.audio_asset_id,
                ...
                caption_refs_ok = bool(
                    ...
                    and expected_caption_ref
                    == f"caption-compilation:{expected_caption_hash}"
                ...
                scene_timing_ok = len(timing_by_scene) == len(plan.scenes) and all(
                    ...
                    == (
                        scene.narration_start_ms,
                        scene.narration_end_ms,
                        scene.duration_ms,
                    )
        ...
        provider_scenes = [
            scene
            for scene in plan.scenes
            if scene.visual_treatment in {"STOCK_VIDEO", "AI_HERO_VIDEO"}
        ]
        if plan.temporal_authority_mode == "CANONICAL_STRICT" and provider_scenes:
            visual_refs_ok = bool(
                plan.visual_direction_contract_ref
                and plan.visual_direction_contract_hash
            )
            results.append(
                self._gate(
        ...
        for scene in plan.scenes:
            ...
            unresolved.extend(
                f"{scene.scene_id}:{req.key}"
                for req in scene.asset_requirements
                if req.required and req.key not in resolved
            )
            if (
                execution
                and not allow_resolved_provider_assets
                and scene.visual_treatment in {"STOCK_VIDEO", "AI_HERO_VIDEO"}
            ):
                unresolved.append(f"{scene.scene_id}:PROVIDER_INTENT_NOT_LOCAL")
        results.append(
            self._gate(
                "AssetResolutionGate", not unresolved, "ASSET_UNRESOLVED", unresolved
            )
        )
        results.append(
            self._gate(
                "OutputProfileGate",
                bool(plan.output_profiles)
                and all(p in ACTIVE_OUTPUT_PROFILES for p in plan.output_profiles),
    ...
    @staticmethod
    def _gate(
        name: str, passed: bool, code: str, details: list[str] | None = None
    ) -> GateResult:
        return GateResult(
            gate=name,
            verdict="PASS" if passed else "BLOCK",
            reason_codes=[] if passed else [code, *(details or [])],
        )
