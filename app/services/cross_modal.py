...
def _hash_body(payload: Mapping[str, Any], field: str = "content_hash") -> str:
    return cross_modal_hash({key: value for key, value in payload.items() if key != field})
...
class SectionCoverageCompiler:
    ...
    def compile(
        ...
        *,
        ...
        if not requirements or any(not _clean(item.get("requirement_id")) for item in requirements):
            raise CrossModalContractError("SECTION_COVERAGE_REQUIRED_ASSIGNMENT_UNITS_MISSING")
        ...
        for index, requirement in enumerate(requirements, start=1):
            ...
            if not proposition:
                raise CrossModalContractError("SECTION_COVERAGE_REQUIREMENT_OBLIGATION_MISSING")
            fields = {
                "information_unit_id": f"iu-{index:03d}",
                "assignment_requirement_refs": [requirement_id],
                "evidence_refs": evidence_refs,
                "editorial_function": str(requirement.get("requirement_type") or "ASSIGNMENT_FULFILLMENT"),
                "proposition": proposition,
                "new_information_delta": f"Fulfill {requirement_id} without restating an earlier unit.",
                "importance": "CORE",
                "factual_risk": "HIGH" if "scope" in requirement_id or requirement_id in {"subject", "question"} else "MEDIUM",
            ...
            unit_models.append(InformationUnit(**fields, content_hash=cross_modal_hash(fields)))
        ...
        for index, unit in enumerate(unit_models):
            grouped[min(section_count - 1, index * section_count // len(unit_models))].append(unit)
        ...
        for ordinal, units in enumerate(grouped, start=1):
            ...
            allocation = max(len(units), round(target_words * len(units) / len(unit_models)))
            body = {
                "section_id": f"section-{ordinal:03d}",
                "ordinal": ordinal,
                "primary_requirement_ids": [ref for unit in units for ref in unit.assignment_requirement_refs],
                "owned_information_unit_ids": [unit.information_unit_id for unit in units],
                "permitted_callbacks": [],
                "word_min": max(1, math.floor(allocation * 0.75)),
                "word_max": max(1, math.ceil(allocation * 1.30)),
                "section_delta": " → ".join(unit.new_information_delta for unit in units),
            }
            sections.append(SectionCoverage(**body, content_hash=cross_modal_hash(body)))
        payload = {
    ...
    def validate_writer_output(
        ...
        if len(by_id) != len(output.sections) or set(by_id) != {
            ...
            raise CrossModalContractError("SCRIPT_SECTION_COVERAGE_SECTION_SET_MISMATCH")
        for coverage in plan.sections:
            section = by_id[coverage.section_id]
            if section.ordinal != coverage.ordinal:
                raise CrossModalContractError("SCRIPT_SECTION_COVERAGE_ORDINAL_MISMATCH")
            if section.required_assignment_unit_refs != coverage.primary_requirement_ids:
                raise CrossModalContractError("SCRIPT_SECTION_COVERAGE_REQUIREMENT_OWNERSHIP_VIOLATION")
...
class NarrationUnitCompiler:
    ...
    def compile(
        ...
        *,
        ...
        info_by_id = {item.information_unit_id: item for item in coverage_plan.information_units}
        ...
        for index, section in enumerate(sections):
            ...
            if start < 0:
                raise CrossModalContractError("NARRATION_UNIT_CANONICAL_SECTION_SPAN_MISSING")
            end = start + len(narration)
            if canonical_script[start:end] != narration:
                raise CrossModalContractError("NARRATION_UNIT_CANONICAL_SECTION_SPAN_INVALID")
            coverage = coverage_by_id[section.section_id]
            chunks = cls._meaning_chunks(narration)
            chunk_cursor = start
            owned_information = [info_by_id[item] for item in coverage.owned_information_unit_ids]
            for chunk_index, chunk in enumerate(chunks):
                ...
                body = {
                    ...
                    "assignment_requirement_ids": list(info.assignment_requirement_refs),
                    ...
                    "estimated_spoken_duration_ms": max(1, round(estimated_duration_ms * word_count / total_words)),
                }
                units.append(NarrationUnit(**body, content_hash=cross_modal_hash(body)))
                ordinal += 1
                chunk_cursor = chunk_end
            cursor = end + (2 if index < len(sections) - 1 else 0)
        ...
        return NarrationUnitCompilation(**payload, content_hash=cross_modal_hash(payload))

    @staticmethod
    def _meaning_chunks(narration: str) -> list[str]:
        sentences = [
            match.group(0).strip()
            for match in re.finditer(r"[^.!?]+(?:[.!?]+|$)", narration, flags=re.S)
        ...
        for sentence in sentences:
            ...
            if (len(pending) >= 2 and words >= 18) or words >= 56:
                ...
                pending, words = [], 0
        if pending:
            if chunks and len(pending) == 1 and _word_count(pending[0]) < 18:
                chunks[-1] = f"{chunks[-1]} {pending[0]}"
            else:
                chunks.append(" ".join(pending))
        return chunks

    @staticmethod
    def _validate_non_whitespace_coverage(script: str, units: list[NarrationUnit]) -> None:
        covered: set[int] = set()
        for unit in units:
            covered.update(range(unit.source_text_span.start, unit.source_text_span.end))
        expected = {index for index, character in enumerate(script) if not character.isspace()}
        if covered & {index for index, character in enumerate(script) if character.isspace()}:
    ...
    *,
    ...
    for ordinal, word in enumerate(words, start=1):
        ...
        try:
            ...
            raise CrossModalContractError("NARRATION_UNIT_TIMED_WORD_FORMAT_INVALID") from exc
        if (
            not token
            or start < 0
            or end <= start
    ...
    for unit in compilation.narration_units:
        ...
        refs = [f"timed-word:{int(item.get('index', cursor + position + 1))}" for position, item in enumerate(matched)]
        body = {
            "narration_unit_id": unit.narration_unit_id,
            "spoken_text_hash": spoken_text_hash,
            "spoken_token_refs": [f"spoken-token:{cursor + position + 1}" for position in range(count)],
    ...
    *,
    ...
    for scene in sorted(plan.scenes, key=lambda item: item.ordinal):
        ...
        if (
            ...
            raise CrossModalContractError("VISUAL_REALIZATION_TIMING_NOT_NARRATION_BOUND")
...
def asset_fit_observation(
    *,
    ...
    required = {
        "subject_match", "action_match", "context_match", "factual_match",
        "composition_match", "timing_fit", "style_compatibility", "visual_redundancy",
    }
    if set(components) != required or any(value not in {"PASS", "WARN", "BLOCK"} for value in components.values()):
        raise CrossModalContractError("ASSET_FIT_COMPONENTS_INVALID")
    disposition = "BLOCK" if "BLOCK" in components.values() else ("WARN" if "WARN" in components.values() else "PASS")
    body = {
        "scene_id": scene_id,
        "asset_ref": asset_ref,
        **dict(components),
...
def cross_modal_qc_report(
    *,
    ...
    if any(item.reason_code in _HARD_BLOCK_CODES and item.severity != "BLOCK" for item in typed):
        raise CrossModalContractError("CROSS_MODAL_HARD_FAILURE_NOT_BLOCKED")
    disposition = "BLOCK" if any(item.severity == "BLOCK" for item in typed) else ("WARN" if any(item.severity == "WARN" for item in typed) else "PASS")
    payload = {
