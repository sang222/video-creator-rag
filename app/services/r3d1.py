...
class ChannelContractAuthorityResolution:
    ...
    def as_ref(self) -> dict[str, Any]:
        return {
            ...
            "channel_profile_version_id": str(self.profile_version.id)
            if self.profile_version
            else None,
            "policy_snapshot_id": str(self.policy_snapshot.id)
...
class CharacterBindingResolution:
    ...
    def as_ref(self) -> dict[str, Any]:
        return {
            ...
            "ok": self.ok,
            "reason_codes": self.reason_codes,
            "character_binding_id": str(self.character_binding.id)
...
class ProjectScopeAdmissionResult:
    ...
    def ok(self) -> bool:
        return (
            self.authority.ok and self.category_scope.ok and self.character_binding.ok
        )
    ...
    def character_binding_id(self) -> uuid.UUID | None:
        return (
            self.character_binding.character_binding.id
            if self.character_binding.character_binding
            else None
        )
    ...
    def resolve(
        ...
        *,
        ...
        if (
            ...
            or profile_version.channel_workspace_id != channel_workspace_id
        ):
            return self._blocked(
                channel=channel,
                policy_snapshot=snapshot,
        ...
        if (
            ...
            or contract_status != CONTRACT_COMPLETE
    ...
    def _blocked(
        ...
        *,
        ...
        return ChannelContractAuthorityResolution(
            ...
            policy_snapshot_content_hash=policy_snapshot.content_hash
            if policy_snapshot
            else None,
...
class CategoryScopeResolver:
    ...
    def resolve_for_editorial_run(
        self,
        editorial_run: EditorialResearchRun,
        *,
        explicit_category_id: uuid.UUID | None = None,
    ) -> CategoryScopeResolution:
        slot = (
            self.session.get(
                EditorialCalendarSlot, editorial_run.editorial_calendar_slot_id
            )
            if editorial_run.editorial_calendar_slot_id
            else None
        )
        category_id = explicit_category_id or (
            slot.category_id if slot is not None else None
        )
        if category_id is not None:
            return self.resolve_explicit(
                company_id=editorial_run.company_id,
                channel_workspace_id=editorial_run.channel_workspace_id,
        ...
        active_categories = list(
            self.session.scalars(
                ...
                .where(ContentCategory.company_id == editorial_run.company_id)
                .where(
                    ContentCategory.channel_workspace_id
                    == editorial_run.channel_workspace_id
                )
        ...
        if len(active_categories) == 1:
            return CategoryScopeResolution(
                ...
                reason_codes=[],
                source="single_active_auto_bind",
            )
        return CategoryScopeResolution(
            category=None,
    ...
    def resolve_explicit(
        ...
        *,
        ...
        if (
            category is None
            or category.company_id != company_id
            or category.channel_workspace_id != channel_workspace_id
        ):
            return CategoryScopeResolution(
                category=None,
                reason_codes=[_code(RuntimeScopeErrorCode.CATEGORY_SCOPE_MISSING)],
        ...
        return CategoryScopeResolution(
            category=category, reason_codes=[], source=source
        )
...
class CharacterBindingResolver:
    ...
    def resolve(
        ...
        *,
        ...
        if mode == NO_CHARACTER:
            if explicit_character_binding_id is not None:
                return CharacterBindingResolution(
                    ...
                    reason_codes=[
                        _code(RuntimeScopeErrorCode.CHARACTER_BINDING_FORBIDDEN)
                    ],
                    source="forbidden_by_category",
                )
            return CharacterBindingResolution(
                character_binding=None, reason_codes=[], source="no_character"
            )
        if explicit_character_binding_id is not None:
            binding = self.session.get(CharacterBinding, explicit_character_binding_id)
            return self._verify_binding(
                category=category, binding=binding, source="explicit"
            )
        if mode == OPTIONAL_CHARACTER:
            return CharacterBindingResolution(
                character_binding=None, reason_codes=[], source="optional_not_bound"
            )
        ...
        return self._verify_binding(
            category=category, binding=binding, source="required_auto_bind"
        )

    def _find_required_binding(
        ...
        category_bindings = list(
            self.session.scalars(
                ...
                .where(
                    CharacterBinding.channel_workspace_id
                    == category.channel_workspace_id
                )
    ...
    def _verify_binding(
        ...
        *,
        ...
        if (
            ...
            or (
                binding.content_category_id is not None
                and binding.content_category_id != category.id
            )
        ):
            return CharacterBindingResolution(
                character_binding=binding,
                reason_codes=[
                    _code(RuntimeScopeErrorCode.CHARACTER_BINDING_NOT_ACTIVE)
                ],
                source=source,
            )
        ...
        if category.character_policy_mode == REQUIRED_CHARACTER:
            readiness_reasons = self._required_readiness_reasons(
                binding=binding, profile=profile, version=version
            )
        ...
        return CharacterBindingResolution(
            character_binding=binding, reason_codes=[], source=source
        )
    ...
    def _required_readiness_reasons(
        ...
        *,
        ...
        reasons: list[str] = []
        branch = (
            self.session.get(CharacterImageBranch, binding.character_image_branch_id)
            if binding.character_image_branch_id
            else None
        )
        if (
            branch is None
            or branch.character_version_id != version.id
            or branch.status != ACTIVE
        ):
            reasons.append(_code(RuntimeScopeErrorCode.CHARACTER_ASSET_PACK_MISSING))
        pack = (
            self.session.get(
                CharacterReferenceAssetPack, binding.reference_asset_pack_id
            )
            if binding.reference_asset_pack_id
            else None
        )
        if (
            pack is None
            ...
            if _code(RuntimeScopeErrorCode.CHARACTER_ASSET_PACK_MISSING) not in reasons:
                reasons.append(
                    _code(RuntimeScopeErrorCode.CHARACTER_ASSET_PACK_MISSING)
                )
        voice = (
            self.session.get(VoiceProfile, binding.voice_profile_id)
        ...
        if (
            ...
            or (
                voice.character_profile_id is not None
                and voice.character_profile_id != profile.id
            )
...
class ProjectScopeAdmissionGuard:
    ...
    def evaluate_for_editorial_run(
        self,
        editorial_run: EditorialResearchRun,
        *,
        explicit_category_id: uuid.UUID | None = None,
        ...
        authority = ChannelRuntimeAuthorityService(self.session).resolve(
            company_id=editorial_run.company_id,
            channel_workspace_id=editorial_run.channel_workspace_id,
            policy_snapshot_id=editorial_run.policy_snapshot_id,
        )
        empty_category = CategoryScopeResolution(
            category=None, reason_codes=[], source="not_evaluated"
        )
        empty_binding = CharacterBindingResolution(
            character_binding=None, reason_codes=[], source="not_evaluated"
        )
        if not authority.ok:
            return ProjectScopeAdmissionResult(
                authority=authority,
                category_scope=empty_category,
                character_binding=empty_binding,
            )
        category_scope = CategoryScopeResolver(self.session).resolve_for_editorial_run(
            editorial_run,
        ...
        binding_id = explicit_character_binding_id or self._slot_character_binding_id(
            editorial_run
        )
    ...
    def _slot_character_binding_id(
        self, editorial_run: EditorialResearchRun
    ) -> uuid.UUID | None:
        if editorial_run.editorial_calendar_slot_id is None:
            return None
...
class R3D1AdminService:
    ...
    def create_content_category(self, data: ContentCategoryCreate) -> ContentCategory:
        ...
        category = ContentCategory(
            **payload, content_hash=content_hash(_hash_payload(data))
        )
        ...
        *,
        ...
        if channel_workspace_id is not None:
            statement = statement.where(
                ContentCategory.channel_workspace_id == channel_workspace_id
            )
    ...
    def create_category_creative_digest(
        self, data: CategoryCreativeDigestCreate
    ) -> CategoryCreativeDigest:
        self._require(ContentCategory, data.content_category_id, "content category")
        payload = _db_payload(data)
        digest = CategoryCreativeDigest(
            **payload, digest_hash=content_hash(_hash_payload(data))
        )
    ...
    def create_character_profile(
        self, data: CharacterProfileCreate
    ) -> CharacterProfile:
        self._require_channel_for_company(data.company_id, data.channel_workspace_id)
        payload = _db_payload(data)
        profile = CharacterProfile(
    ...
    def create_character_version(
        self, data: CharacterVersionCreate
    ) -> CharacterVersion:
    ...
    def create_character_image_branch(
        self, data: CharacterImageBranchCreate
    ) -> CharacterImageBranch:
        self._require(CharacterVersion, data.character_version_id, "character version")
        payload = _db_payload(data)
        branch = CharacterImageBranch(
            **payload, content_hash=content_hash(_hash_payload(data))
        )
    ...
    def create_character_reference_asset_pack(
        ...
        self._require(
            CharacterImageBranch,
            data.character_image_branch_id,
            "character image branch",
        )
        payload = _db_payload(data)
        pack = CharacterReferenceAssetPack(
            **payload, content_hash=content_hash(_hash_payload(data))
        )
    ...
    def create_character_reference_asset(
        self, data: CharacterReferenceAssetCreate
    ) -> CharacterReferenceAsset:
        self._require(
            CharacterReferenceAssetPack,
            data.reference_asset_pack_id,
    ...
    def create_voice_profile(self, data: VoiceProfileCreate) -> VoiceProfile:
        ...
        if data.character_profile_id is not None:
            profile = self._require(
                CharacterProfile, data.character_profile_id, "character profile"
            )
            if (
                ...
                raise ValidationFailureError(
                    "voice profile character does not belong to channel scope"
                )
        payload = _db_payload(data)
    ...
    def create_character_binding(
        self, data: CharacterBindingCreate
    ) -> CharacterBinding:
        self._require_channel_for_company(data.company_id, data.channel_workspace_id)
        if data.content_category_id is not None:
            category = self._require(
                ContentCategory, data.content_category_id, "content category"
            )
            if (
                ...
                raise ValidationFailureError(
                    "binding category does not belong to channel scope"
                )
        profile = self._require(
            CharacterProfile, data.character_profile_id, "character profile"
        )
        version = self._require(
            CharacterVersion, data.character_version_id, "character version"
        )
        if (
            profile.company_id != data.company_id
            or profile.channel_workspace_id != data.channel_workspace_id
            ...
            raise ValidationFailureError(
                "binding character version does not belong to channel scope"
            )
        if data.character_image_branch_id is not None:
            ...
            if branch.character_version_id != version.id:
                raise ValidationFailureError(
                    "binding image branch does not belong to character version"
                )
        if data.reference_asset_pack_id is not None:
            ...
            if (
                data.character_image_branch_id is not None
                and pack.character_image_branch_id != data.character_image_branch_id
            ):
                raise ValidationFailureError(
                    "binding asset pack does not belong to image branch"
                )
        if data.voice_profile_id is not None:
            ...
            if (
                ...
                raise ValidationFailureError(
                    "binding voice profile does not belong to character scope"
                )
        payload = _db_payload(data)
        binding = CharacterBinding(
            **payload, content_hash=content_hash(_hash_payload(data))
    ...
    def list_character_versions(
        self, *, character_profile_id: uuid.UUID | None = None
        ...
        if character_profile_id is not None:
            statement = statement.where(
                CharacterVersion.character_profile_id == character_profile_id
            )
        return list(self.session.scalars(statement).all())

    def list_character_image_branches(
        self, *, character_version_id: uuid.UUID | None = None
    ) -> list[CharacterImageBranch]:
        statement = select(CharacterImageBranch).order_by(
            CharacterImageBranch.created_at.asc()
        )
        if character_version_id is not None:
            statement = statement.where(
                CharacterImageBranch.character_version_id == character_version_id
            )
        return list(self.session.scalars(statement).all())
    ...
    def list_character_reference_asset_packs(
        ...
        *,
        ...
        statement = select(CharacterReferenceAssetPack).order_by(
            CharacterReferenceAssetPack.created_at.asc()
        )
        if character_image_branch_id is not None:
            statement = statement.where(
                CharacterReferenceAssetPack.character_image_branch_id
                == character_image_branch_id
            )
        return list(self.session.scalars(statement).all())
    ...
    def list_character_reference_assets(
        ...
        *,
        ...
        statement = select(CharacterReferenceAsset).order_by(
            CharacterReferenceAsset.created_at.asc()
        )
        if reference_asset_pack_id is not None:
            statement = statement.where(
                CharacterReferenceAsset.reference_asset_pack_id
                == reference_asset_pack_id
            )
        return list(self.session.scalars(statement).all())
    ...
    def list_character_bindings(
        ...
        *,
        ...
        if channel_workspace_id is not None:
            statement = statement.where(
                CharacterBinding.channel_workspace_id == channel_workspace_id
            )
    ...
    def get_character_image_branch(
        self, record_id: uuid.UUID
    ) -> CharacterImageBranch | None:
        return self.session.get(CharacterImageBranch, record_id)

    def get_character_reference_asset_pack(
        self, record_id: uuid.UUID
    ) -> CharacterReferenceAssetPack | None:
        return self.session.get(CharacterReferenceAssetPack, record_id)

    def get_character_reference_asset(
        self, record_id: uuid.UUID
    ) -> CharacterReferenceAsset | None:
    ...
    def _list_scoped(
        ...
        *,
        ...
        if channel_workspace_id is not None:
            statement = statement.where(
                model.channel_workspace_id == channel_workspace_id
            )
        return list(self.session.scalars(statement).all())
