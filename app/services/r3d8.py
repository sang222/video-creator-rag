...
CHARACTER_DEPENDENT_STAGES = {
    "AI_HERO_VIDEO",
    "AI_HERO_GENERATION",
    "AI_METAPHOR_GENERATION",
}
VOICE_STAGES = {"VOICE_GENERATION", "LONG_VOICE_GENERATION"}
...
def derive_google_gemini_image_catalog_cost(
    ...
    *,
    ...
    reasons: list[str] = []
    provider_stage = (
        str(item.get("provider_stage") or item.get("stage") or "").strip().upper()
    )
    ...
    if (
        ...
        or str(item.get("currency") or "USD").upper() != "USD"
    ):
    ...
    if (
        submitted_estimate is not None
        and submitted_estimate != estimate.estimated_amount
    ):
        return GateCheck(
            False,
...
class RenderRevisionService:
    ...
    def create(self, data: RenderRevisionCreateRequest) -> RenderRevision:
        ...
        source_refs = data.source_artifact_refs_json or _artifact_refs_from_package(
            package
        )
        ...
        revision_no = (
            int(
                self.session.scalar(
                    select(
                        func.coalesce(func.max(RenderRevision.revision_no), 0)
                    ).where(RenderRevision.package_id == package.id)
                )
                or 0
            )
            + 1
    ...
    def list_for_package(self, package_id: uuid.UUID) -> list[RenderRevision]:
        return list(
            self.session.scalars(
                ...
                .order_by(
                    desc(RenderRevision.revision_no), desc(RenderRevision.created_at)
                )
            )
        )

    def _latest_gate_batch_refs(
        ...
        return [
            {"gate_batch_run_id": str(batch.id), "status": batch.status}
...
class CostEstimateService:
    ...
    def create(self, data: CostEstimateCreateRequest) -> CostEstimateSnapshot:
        ...
        for item in provider_items:
            provider_key = (
                normalize_provider_key(item.get("provider_key"))
                or str(item.get("provider_key") or "").strip().lower()
            )
            provider_stage = (
                str(item.get("provider_stage") or item.get("stage") or "")
                .strip()
                .upper()
            )
            ...
            provider_estimates[key] = {
                ...
                "readiness_state": "STALE_PROVIDER_KEY"
                if stale_reasons
                else provider.readiness_state
                if provider
                else "UNKNOWN",
                "estimated_cost": (
                    None
            ...
            if provider_key == "pexels_api":
                ...
                if not item.get("attribution_manifest_ref") or not item.get(
                    "usage_policy_manifest_ref"
                ):
            ...
            if provider_key in PAID_PROVIDER_KEYS and explicit_cost is None:
                blockers.append(
                    f"{provider_key.upper()}_ESTIMATE_REQUIRES_REAL_PROVIDER"
                )
        ...
        if not provider_items:
            ...
            code.endswith("_ESTIMATE_REQUIRES_REAL_PROVIDER") for code in blockers
        ):
        ...
        if estimate_status == "ESTIMATED":
            estimated_total = (
                sum((value or Decimal("0")) for value in costs.values())
                + supplemental_paid_cost
            )
        snapshot_payload = {
    ...
    ) -> CostEstimateSnapshot | None:
        return self.session.scalars(
            select(CostEstimateSnapshot)
            .where(CostEstimateSnapshot.render_revision_id == revision_id)
            .order_by(
                desc(CostEstimateSnapshot.created_at), desc(CostEstimateSnapshot.id)
            )
            .limit(1)
        ).one_or_none()
...
class HumanPaidRenderApprovalService:
    ...
    def create_pending(
        self, data: HumanPaidRenderApprovalCreateRequest
    ) -> HumanPaidRenderApproval:
    ...
    def approve(
        self, approval_id: uuid.UUID, data: HumanPaidRenderApprovalDecisionRequest
    ) -> HumanPaidRenderApproval:
...
class ProviderIdempotencyService:
    ...
    def get_or_create(
        self, data: ProviderIdempotencyKeyCreateRequest
    ) -> ProviderIdempotencyKey:
        RenderRevisionService(self.session).get(data.render_revision_id)
    ...
    def record_attempt(
        ...
        *,
        ...
        if record.attempt_count >= record.max_attempts:
            ...
            record.status = (
                "PASS" if record.attempt_count <= record.max_attempts else "BLOCKED"
            )
...
class PaidRenderApprovalGate:
    ...
    def check(
        ...
        *,
        ...
        if approval is None:
            return GateCheck(
                False, ["HUMAN_PAID_APPROVAL_MISSING"], "WAITING_HUMAN_PAID_APPROVAL"
            )
        if approval.render_revision_id != revision.id:
            return GateCheck(
                False,
                ["HUMAN_PAID_APPROVAL_REVISION_MISMATCH"],
                "WAITING_HUMAN_PAID_APPROVAL",
            )
        if approval.approval_status != "APPROVED":
            return GateCheck(
                False,
                [f"HUMAN_PAID_APPROVAL_{approval.approval_status}"],
                "WAITING_HUMAN_PAID_APPROVAL",
            )
    ...
    def check(
        ...
        *,
        ...
        requires_character = bool(
            request_payload.get("requires_character")
            or request_payload.get("character_ref_required")
        )
        payload_refs = {
            "character_profile_id": request_payload.get("character_profile_id"),
            "character_image_branch_id": request_payload.get(
                "character_image_branch_id"
            ),
            "reference_asset_pack_id": request_payload.get("reference_asset_pack_id"),
        }
        if effective.character_policy_mode == "NO_CHARACTER" and (
            requires_character or any(payload_refs.values())
        ):
        ...
        expected = {
            "character_profile_id": str(effective.character_profile_id)
            if effective.character_profile_id
            else None,
            "character_image_branch_id": str(effective.character_image_branch_id)
            if effective.character_image_branch_id
            else None,
            "reference_asset_pack_id": str(effective.reference_asset_pack_id)
            if effective.reference_asset_pack_id
            else None,
        }
        ...
        return GateCheck(
            not reasons,
            sorted(set(reasons)),
            "PASS" if not reasons else "BLOCKED_CHARACTER_INPUT",
        )
...
class ProviderVoiceInputGate:
    ...
    def check(
        ...
        *,
        ...
        reasons: list[str] = []
        voice_profile_id = (
            request_payload.get("voice_profile_id") or effective.voice_profile_id
        )
        ...
        if effective.voice_profile_id and str(voice_profile_id) != str(
            effective.voice_profile_id
        ):
        ...
        if voice is None:
            ...
            if voice.commercial_use_status not in {
                "ALLOWED",
                "APPROVED",
                "NOT_REQUIRED",
            }:
        ...
        return GateCheck(
            not reasons,
            sorted(set(reasons)),
            "PASS" if not reasons else "BLOCKED_VOICE_INPUT",
        )
...
class PexelsUsagePolicyGate:
    ...
    def check(self, *, request_payload: dict[str, Any]) -> GateCheck:
        ...
        return GateCheck(
            not reasons, sorted(set(reasons)), "PASS" if not reasons else "BLOCK"
        )
...
class VisualSourceMixGate:
    ...
    def check(
        ...
        *,
        ...
        reasons: list[str] = []
        if (
            str(plan.get("visual_backbone") or "").upper() == "PEXELS"
            or plan.get("pexels_core_backbone") is True
        ):
        ...
        if provider_key == "google_veo" or stage in CHARACTER_DEPENDENT_STAGES:
            veo_duration = _decimal_or_none(
                plan.get("ai_hero_duration_seconds") or plan.get("duration_seconds")
            )
...
class ProxyPreviewGate:
    ...
    def flag(
        self, data: ProxyPreviewArtifactFlagCreateRequest
    ) -> ProxyPreviewArtifactFlag:
        if (
            not data.preview_only
            or not data.not_final_media
            or not data.not_publishable
        ):
    ...
    def check_not_final_media(self, *, artifact_ref: str | None) -> GateCheck:
        ...
        if record.preview_only or record.not_final_media or record.not_publishable:
            return GateCheck(
                False,
                ["PROXY_PREVIEW_ARTIFACT_NOT_PUBLISHABLE"],
...
class PaidProviderBoundaryService:
    ...
    def preflight(
        self, data: ProviderBoundaryPreflightRequest
    ) -> ProviderBoundaryDecisionRead:
        revision = RenderRevisionService(self.session).get(data.render_revision_id)
        effective = self.session.get(
            EffectiveChannelRuntimeContextSnapshot,
            revision.effective_context_snapshot_id,
        )
        if effective is None:
            raise NotFoundError(
                f"effective context not found: {revision.effective_context_snapshot_id}"
            )
        ...
        estimate = self._estimate(
            revision=revision, estimate_id=data.cost_estimate_snapshot_id
        )
        approval = (
            self.session.get(HumanPaidRenderApproval, data.human_approval_id)
            if data.human_approval_id
            else self._latest_approved_revision_approval(revision)
        )
        reasons: list[str] = provider_key_rejection_reasons(provider_key)
        status = (
            "BLOCKED_PROVIDER_NOT_CONFIGURED"
            if reasons
            else "READY_FOR_PROVIDER_BOUNDARY"
        )
        ...
        if provider_key == "pexels_api" or provider_stage in PEXELS_STAGES:
            pexels_check = PexelsUsagePolicyGate(self.settings).check(
                request_payload=data.request_payload_json
            )
            if not pexels_check.passed:
                ...
                status = (
                    "BLOCKED_PROVIDER_NOT_CONFIGURED"
                    if "PEXELS_API_KEY_MISSING" in pexels_check.reason_codes
                    else "BLOCKED_COST_ESTIMATE"
                )
        ...
        if not visual_check.passed:
            ...
            status = (
                ...
                if "VEO" in " ".join(visual_check.reason_codes)
                else "BLOCKED_COST_ESTIMATE"
            )
        proxy_check = ProxyPreviewGate(self.session).check_not_final_media(
            artifact_ref=data.request_payload_json.get("final_artifact_ref")
        )
    ...
    def _provider_ready(self, provider_key: str) -> GateCheck:
        ...
        if provider.readiness_state not in {
            ...
            return GateCheck(
                False,
                ["BLOCKED_PROVIDER_NOT_CONFIGURED", *provider.blocker_reason_codes],
                "BLOCKED_PROVIDER_NOT_CONFIGURED",
            )
        return GateCheck(True, ["PROVIDER_CONFIGURED"], "PASS")

    def _estimate(
        self, *, revision: RenderRevision, estimate_id: uuid.UUID | None
    ) -> CostEstimateSnapshot | None:
        if estimate_id:
            ...
            return estimate
        return CostEstimateService(self.session, self.settings).latest_for_revision(
            revision.id
        )

    def _latest_approved_revision_approval(
        self, revision: RenderRevision
    ) -> HumanPaidRenderApproval | None:
        return self.session.scalars(
            ...
            .order_by(
                desc(HumanPaidRenderApproval.created_at),
                desc(HumanPaidRenderApproval.id),
            )
            .limit(1)
        ).one_or_none()
    ...
    def _deterministic_gate_check(self, revision: RenderRevision) -> GateCheck:
        ...
        for ref in _list(revision.gate_batch_refs_json):
            ...
            batch = (
                self.session.get(R3D4GateBatchRun, uuid.UUID(str(ref_id)))
                if ref_id
                else None
            )
        ...
        return GateCheck(
            not reasons,
            sorted(set(reasons)),
            "PASS" if not reasons else "BLOCKED_DETERMINISTIC_GATE",
        )
...
class ProviderJobService:
    ...
    def create_not_submitted(
        self, data: ProviderJobCreateRequest
    ) -> ProviderJobSnapshot:
        RenderRevisionService(self.session).get(data.render_revision_id)
    ...
    def mark_timeout_resume_required(
        self, job_snapshot_id: uuid.UUID
    ) -> ProviderJobSnapshot:
        ...
        snapshot.last_error_message = (
            "Timeout requires resume/poll continuation; do not submit duplicate job."
        )
        self.session.flush()
        return snapshot


def _artifact_refs_from_package(
...
def _planned_provider_items(provider_plan: dict[str, Any]) -> list[dict[str, Any]]:
    if isinstance(provider_plan.get("provider_stages"), list):
        return [
            item for item in provider_plan["provider_stages"] if isinstance(item, dict)
        ]
