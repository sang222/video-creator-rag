...
class PKG1SC04RevisionApprovalCommand(BaseModel):
    ...
    reviewed_revision_version: Literal[3]
    ...
    approval_scope: Literal["PKG1_SC04_REVISION_PACKAGE_PLANNING"] = (
        "PKG1_SC04_REVISION_PACKAGE_PLANNING"
    )
    approval_ref: str
    ...
    def validate_approval_ref(cls, value: str) -> str:
        if not value.startswith("operator-approval://pkg1-sc04-revision/"):
            raise ValueError(
                "approval_ref must identify the PKG1 SC-04 revision operator decision"
            )
        return value
