...
def _replace_observation_window_constraint(*, include_long_form: bool) -> None:
    ...
    for table_name in tables:
        _drop_observation_window_constraints(table_name)
        legacy = (
            "'T_PLUS_1H','T_PLUS_6H','T_PLUS_24H','T_PLUS_48H','T_PLUS_7D','CUSTOM'"
        )
        values = (
            f"{legacy},{LONG_FORM_OBSERVATION_WINDOWS}" if include_long_form else legacy
        )
...
def _drop_observation_window_constraints(table_name: str) -> None:
    """Resolve legacy names online, or defer that lookup to offline SQL runtime."""
...
def _assert_no_long_form_observation_data() -> None:
    predicate = (
        "EXISTS (SELECT 1 FROM analytics_snapshots "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM post_publish_observation_windows "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM post_publish_health_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM no_view_diagnostic_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM packaging_diagnostic_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM retention_diagnostic_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM engagement_diagnostic_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM policy_rights_diagnostic_runs "
        "WHERE observation_window in ('H24','H72','D7','D30')) "
        "OR EXISTS (SELECT 1 FROM failure_trace_reports "
        "WHERE observation_window in ('H24','H72','D7','D30'))"
    )
    if context.is_offline_mode():
        op.execute(
            sa.text(
                ...
                BEGIN
                    IF {predicate} THEN
                        RAISE EXCEPTION
                            'DOWNGRADE_BLOCKED_LONG_FORM_ANALYTICS_OBSERVATIONS_EXIST';
                    END IF;
                END;
                $$;
                """
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "production_lane = 'LONG_FORM'", name="ck_lfa_windows_long_form_only"
        ),
        sa.CheckConstraint(
            "window_type in ('H24','H72','D7','D30')", name="ck_lfa_windows_type"
        ),
        sa.CheckConstraint(
            "metric_authority in ('YOUTUBE_OWNER','YOUTUBE_PUBLIC','MANUAL_VERIFIED')",
            name="ck_lfa_windows_authority",
        ),
        sa.CheckConstraint(
            "state in ('SCHEDULED','WAITING_FOR_MATURITY','READY_TO_SYNC','SYNCING','SYNCED','DIAGNOSTICS_PENDING','DIAGNOSTICS_COMPLETE','RETRY_SCHEDULED','BLOCKED_AUTH','BLOCKED_DATA_UNAVAILABLE','FAILED_TERMINAL','CANCELED')",
            name="ck_lfa_windows_state",
        ),
        sa.CheckConstraint(
            "attempt_count >= 0 and max_attempts > 0 and attempt_count <= max_attempts",
            name="ck_lfa_windows_attempts",
        ),
        sa.CheckConstraint(
            "(content_mode = 'SERIES_EPISODE' and series_plan_id is not null and series_run_id is not null and episode_number > 0 and standalone_reason_code is null) or (content_mode = 'STANDALONE' and series_plan_id is null and series_run_id is null and episode_number is null and standalone_reason_code is not null)",
            name="ck_lfa_windows_assignment",
        ),
        ...
        sa.ForeignKeyConstraint(
            ["post_publish_health_run_id"], ["post_publish_health_runs.id"]
        ),
    ...
    op.create_index(
        ...
        "long_form_analytics_windows",
        ["state", "scheduled_for", "next_attempt_at"],
    )
    op.create_index(
        "ix_long_form_analytics_windows_uploaded",
        "long_form_analytics_windows",
        ["uploaded_video_id"],
    )
    op.create_index(
        "ix_long_form_analytics_windows_channel",
        "long_form_analytics_windows",
...
def downgrade() -> None:
    if context.is_offline_mode():
        op.execute(
            sa.text(
                ...
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM long_form_analytics_windows) THEN
                        RAISE EXCEPTION
                            'DOWNGRADE_BLOCKED_LONG_FORM_ANALYTICS_AUTHORITY_EXISTS';
                    END IF;
                END;
                $$;
                """
        ...
        op.get_bind()
        .execute(sa.text("select count(*) from long_form_analytics_windows"))
        .scalar_one()
    ):
    ...
    op.drop_index(
        "ix_long_form_analytics_windows_channel",
        table_name="long_form_analytics_windows",
    )
    op.drop_index(
        "ix_long_form_analytics_windows_uploaded",
        table_name="long_form_analytics_windows",
    ...
    op.drop_index(
        "ix_long_form_analytics_windows_due", table_name="long_form_analytics_windows"
    )
