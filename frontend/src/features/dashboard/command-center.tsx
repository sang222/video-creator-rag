        ...
        {data.cards.length ? (
          ...
          <div className="mt-5">
            <EmptyStateCard
              ...
              description="Backend chưa gửi action card nào cho phiên này. Khi có hàng chờ, cảnh báo hoặc việc cần người vận hành, VCOS sẽ đưa lên đầu Trung tâm."
              actions={[{ label: "Xem hàng chờ duyệt", href: "/queues" }]}
            />
          </div>
        )}
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <YAxis allowDecimals={false} stroke="#a1a8b3" />
                  <Tooltip cursor={{ fill: "rgba(255,255,255,0.04)" }} contentStyle={{ background: "#1b1f2a", border: "1px solid #2a3040" }} />
                  <Bar dataKey="count" fill="#2fc7a3" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <p className="text-sm leading-6 text-muted-foreground">Chưa có action card để vẽ tải việc. Đây là trạng thái bình thường khi hệ thống chưa có việc cần người vận hành.</p>
          )}
        </Panel>
        <div className="grid gap-4">
          {data.metrics.length ? (
            ...
            <ActionHintCard
              title="Chưa có metric vận hành"
              body="Khi backend có số liệu thật, thẻ metric sẽ hiển thị ở đây. VCOS không tự suy diễn xu hướng hoặc hiệu quả."
              href="/uploaded-videos"
              actionLabel="Xem video đã upload"
            />
          )}
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {data.safety_warnings.length ? (
          data.safety_warnings.map((warning) => (
            <Panel key={warning.key} className="border-amber-500/30 bg-amber-500/5">
          ...
          <EmptyStateCard
            title="Chưa có cảnh báo vận hành"
            description="Không có cảnh báo nghĩa là backend chưa gửi quy tắc cần nhắc trong phiên này. Các quy tắc an toàn vẫn được giữ ở từng luồng liên quan."
            actions={[{ label: "Xem cài đặt", href: "/settings" }]}
          />
        )}
      </section>
    </div>
