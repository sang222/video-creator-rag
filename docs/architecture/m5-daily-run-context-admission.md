...
- ChannelAuthorityService routed through the registered `DailyIdeaAgent`, Prompt Registry, and `LLMRouter` boundary.
...
- New direct provider integrations, direct provider SDK calls, or unguarded external network calls. Daily authority uses the existing guarded LLMRouter boundary only.
...
1. Create an explicit `editorial_calendar_slot` or use the legacy slot-optional path. A CH1-FLEX v2 production slot must bind category, pillar, series, production goal, active profile, and active policy snapshot.
...
5. `DailyIdeaAgent` is rendered through Prompt Registry and routed through `LLMRouter`; a DailyIdeaDecision is created only from schema-valid, bounded output and an auditable LLMRunSnapshot.
6. IdeaMarketPreflight performs deterministic demand checks plus NICH1 topic/channel-fit checks for a strict v2 snapshot.
7. ProjectAdmissionDecision deterministically admits, blocks, skips, or requires review. A strict admission freezes niche/channel/slot lineage before any downstream runtime context is compiled.
...
For a strict CH1-FLEX v2 daily run, allowed source `niche_contract_digest` causes ResourceResolverService to compile the authoritative `NicheContractDigest` from the active frozen channel contract, profile, policy snapshot, ContentCategory, and EditorialCalendarSlot. The persisted pack contains bounded semantic content, not only a hash:
...
- the `DailyIdeaAgent` context contract and PromptBudgetGate report.
...
## DailyIdeaAgent LLM workflow

ChannelAuthorityService delegates to `LLMWorkflowService`. The service accepts only `provider_key=llm_router`, renders the repo-registered `DailyIdeaAgent` template on lane `cheap_structured`, validates the BaseEnvelope and typed daily-idea artifact, then records the LLM run/attempt/cost firewall evidence exposed by the router path.
...
Structured output must contain the topic/angle/format/pillar/series plus `channel_fit_score` and bounded `channel_fit_evidence`. It is strict Pydantic-validated before it can become DailyIdeaDecision or draft ArtifactVersion content. Malformed output, schema mismatch, unavailable route, quota block, or budget block fails safely and creates no VideoProject.
...
DailyIdeaDecision is immutable admission input and remains at its original proposal status. ProjectAdmissionDecision is the transition authority. Candidate VideoProject creation, Effective Context compilation, and initial artifacts run inside one nested transaction; a non-PASS Effective Context rolls the candidate back and persists one idempotent BLOCK admission receipt instead of leaking a partial project.
...
The admitted project freezes profile/policy/channel-contract refs and hashes, `NicheContractDigest`, DailyIdeaDecision, EditorialSlot, category, pillar, series, production goal, topic/angle, topic-gate evidence, and the pre-admission `NicheAlignmentDossier`. Existing admissions reuse their exact frozen preflight and never resolve a newer row. Effective Context later validates this frozen lineage and blocks stale or cross-channel bindings.
