...
Media provider routing truth is role-bound and capability-gated. `media_provider_role_profiles` stores the Quality-First $250 provider role matrix. `provider_capability_matrix_entries` declares supported, unsupported, blocked-by-plan, required-upgrade, external-provider-required, and mock-only job capabilities. `media_render_routing_decisions` preserves every deterministic provider route or block. M10.4 binds `AI_VIDEO_HERO_PROVIDER` to `GOOGLE_VERTEX_VEO` only; Runway, Luma, generic cinematic fallback, web-app-only providers, and backup AI hero auto-route are not configured. Veo model/duration/resolution/cost/budget defaults live in config catalogs with env override, while Google project/location/service account path and real execution flags live in env/secret-store only. Creatomate Essential 2K is `CLOUD_TEMPLATE_RENDERER_LIGHT` and cannot be the default long-form final render backbone. `LONG_FORM_FINAL_RENDER` must return `BLOCKED_PROVIDER_CAPABILITY_REQUIRED` unless a capable `CLOUD_FINAL_ASSEMBLY_RENDERER` or explicit upgraded Creatomate final-render profile is configured. `long_form_render_packages`, `short_render_packages`, `ai_hero_assets`, `creatomate_render_assets`, and `thumbnail_variants` are planning/package truth by default and do not prove real provider output unless an explicitly guarded provider execution returns a real ref. `final_media_refs` require actual known file refs or test fixture refs. `license_evidence_records` preserve license review state for stock/free/manual assets.
