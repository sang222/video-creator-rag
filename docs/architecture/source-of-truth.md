...
LLM router truth is lane-bound and guard-controlled. `llm_router_profiles`, `llm_router_lanes`, and `llm_model_profiles` define the OpenAI Luna/Terra mapping by lane. Business services must request a lane, not a runtime model. `llm_route_attempts` records selected model, reasoning effort, status, hashes, usage/cost metadata, and refs to `provider_attempts` and `llm_run_snapshots` when created. Real OpenAI execution is disabled unless the explicit environment guard and credential are enabled; tests and normal runs do not require a provider call. Token-based cost is settled only from returned Responses usage under the pinned pricing receipt.
...
`llm_run_snapshots` captures M5 mock LLM proposal attempts, M6 mock script draft attempts, and M10.1/M12.1 guarded OpenAI router attempts. LLM output is proposal/draft/rationale/assistant text only and cannot approve, publish, compute metrics, or become numeric truth. Real OpenAI calls are disabled by default. Production prompt calls preserve `system` and `user` roles through the Responses input; legacy raw prompt callers remain supported.
